...
const getUserPosts = async (userId: string) => {
  ...
  return fetch(`${apiUrl}/posts?userId=${userId}`).then((res) => res.json());
};

const getUserAlbums = async (userId: string) => {
  await new Promise((resolve) => setTimeout(resolve, 1000));

  return fetch(`${apiUrl}/albums?userId=${userId}`).then((res) => res.json());
};
...
export default async function UserParallelPage({ params }: Props) {
